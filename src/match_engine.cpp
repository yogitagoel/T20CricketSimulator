...
void MatchEngine::print_final_scorecard() const {
     auto print_bat = [](const std::vector<Player>& bat, int inn,
         ...
         printf(COL_CYAN
                ...
                "|____________INNINGS %d BATTING SCORECARD_________________________________________|\n"
                "|  %-20s | %-5s   | %-6s  | %-9s | %-20s   |\n",
         ...
         for (const auto& p : bat) {
             if (p.balls_faced > 0 || p.is_out) {
                 ...
                 printf("|  %-20s |  %-5d  | %-6d  | %-8.1f%%  | %-20s  |\n",
                        p.name.c_str(), p.runs_scored, p.balls_faced, sr,
                        ds.c_str());
             } else {
                 printf("|  %-20s |  %-5s  | %-6s  | %-9s  | %-20s  |\n",
                        p.name.c_str(), "-", "-", "-", "dnb");
             }
         }
         printf("|________________________________________________________________________________|\n");
     ...
     auto print_bowl = [](const std::vector<Player>& bowl, int inn) {
         printf(COL_MAGENTA
                ...
                "|_____________INNINGS %d BOWLING FIGURES________________________________|\n"
                "|  %-20s |  %-5s  | %-4s  | %-4s  | %-7s  | %-7s |\n",
    ...
    // sorted by FCFS (natural order) and SJF (ascending expected_balls).
     ...
     auto analyse_team = [&](const std::vector<Player>& bat,
         ...
         printf(COL_MAGENTA
                ...
                "|____|______________________|_______|____________|____________|__________|\n",
                team_name.c_str());
