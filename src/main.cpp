...
static void print_help(const char* prog) {
    printf(
        ...
        "  --team1=<n>                  First team  (default: Mumbai Indians)\n"
        "  --team2=<n>                  Second team (default: Chennai Super Kings)\n"
        "  --overs=<n>                  Number of overs (default: 5)\n"
        "  --deadlock                   Simulate run-out deadlock\n"
        "  --gantt                      Generate Gantt chart CSV\n"
        "  --analysis                   Print SJF vs FCFS wait-time analysis\n"
        "  --speed=<n>                  Speed 1=slow, 5=fast, 10=instant\n"
        "  --verbose                    Debug-level output\n"
        "  --log=<file>                 Log file path\n"
        "  --seed=<n>                   RNG seed\n"
        "  --help                       Show this help\n\n"
        "Examples:\n"
        "  %s --scheduler=PRIORITY --overs=5 --deadlock\n"
...
int main(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (strcmp(arg,"--help")==0 || strcmp(arg,"-h")==0) {
            ...
            std::replace(cfg.team1_name.begin(), cfg.team1_name.end(), '_', ' ');
        } else if (starts_with(arg,"--team2=")) {
            cfg.team2_name = std::string(arg + strlen("--team2="));
            std::replace(cfg.team2_name.begin(), cfg.team2_name.end(), '_', ' ');
    ...
    // Toss
    int toss_winner = toss(cfg.team1_name, cfg.team2_name);
    if (toss_winner != 0) std::swap(cfg.team1_name, cfg.team2_name);
    ...
    printf(COL_ORANGE
         ...
         : cfg.scheduler_type == CRICKET_SCHED_SJF ? "SJF" : "Priority",
           ...
           cfg.ball_delay_ms, seed, cfg.log_file.c_str());
    ...
    if (cfg.enable_gantt)
        printf(COL_CYAN
               "  Gantt chart: pip install matplotlib && "
